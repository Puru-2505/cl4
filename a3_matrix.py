...
def matrix_multiply_mapper(row, columns):
    result = 0
    for i in range(len(row)):
        result += row[i] * columns[i]
    return result

def matrix_multiply_worker(args):
    row_index, row, matrix2 = args
    return [(row_index, col_index, matrix_multiply_mapper(row, col)) for col_index, col in enumerate(zip(*matrix2))]
